...
SECTION_ID = "section_86_11"
SECTION_NAME = "卓工学院-新闻动态"
BASE_URL = "https://nee.bjtu.edu.cn/xwdt/"
MAX_PAGES = 1
...
def parse_page(
    ...
    news_list = soup.find("div", class_="subXinwenImgList")
    if news_list is None:
        logger.debug("第 %s 页未找到 div.subXinwenImgList", page)
        return []
...
def _parse_list_item(item: BeautifulSoup, base_url: str) -> ResultSummary | None:
    ...
    if not isinstance(href,str):
        return None
    title_node = item.find("p",class_="subXinwenImgItemTextTittle",recursive=True)
...
def _parse_date(item: BeautifulSoup) -> str | None:
    date_node = item.find("div", class_="subXinwenImgItemTextDate",recursive=True)
