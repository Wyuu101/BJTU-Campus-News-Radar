...
SECTION_ID = "section_81_03"
SECTION_NAME = "经管学院-活动预报"

BASE_URL = "https://sem.bjtu.edu.cn/lists-sem_hdyb.html"
...
def _parse_list_item(item: BeautifulSoup, base_url: str) -> ResultSummary | None:
    ...
    content = item.find("div",class_="fl p_em_box")
    if content is None:
        return None
    href_node = content.find("a",class_="tit_em mb20")
    if href_node is None:
        return None
    href = href_node.get("href")
    if  not isinstance(href, str) or not href:
            return None
    nodes = href_node.find_all("span")
    if nodes is None:
        return None
    if len(nodes) <3:
        return None
    type = nodes[0].get_text("",strip=True)
    title = f"{type} | " + nodes[2].get_text("",strip=True)
