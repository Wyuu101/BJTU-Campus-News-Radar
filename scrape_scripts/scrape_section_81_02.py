...
SECTION_ID = "section_81_02"
SECTION_NAME = "经管学院-招生信息"
...
def _parse_list_item(item: BeautifulSoup, base_url: str) -> ResultSummary | None:
    ...
    if len(content) < 2:
        return None
    type_node = content[0].find("span")
    if type_node is None:
        return None
    type = type_node.get_text(" ",strip=True)

    href_and_short_title_node  = content[1]
    href = href_and_short_title_node.get("href")
    title = f"{type} | " + href_and_short_title_node.get_text(" ",strip=True)
