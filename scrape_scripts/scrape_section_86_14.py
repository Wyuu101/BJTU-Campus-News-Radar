...
SECTION_ID = "section_86_14"
SECTION_NAME = "卓工学院-学校文件"
BASE_URL = "https://nee.bjtu.edu.cn/sjcg/xxwj/"
MAX_PAGES = 1
...
def parse_page(
    ...
    news_list = soup.find("div", class_="subTongzhiTextList")
    if news_list is None:
        logger.debug("第 %s 页未找到 div.subTongzhiTextList", page)
        return []
    results: list[ResultSummary] = []
    for item in news_list.find_all("a",href=True, recursive=False):
...
def _parse_list_item(item: BeautifulSoup, base_url: str) -> ResultSummary | None:
    ...
    if not isinstance(href,str):
        return None
    title_node = item.find("div",class_="subTongzhiTextItemTittle",recursive=True)
...
def _parse_date(item: BeautifulSoup) -> str | None:
    date_node = item.find("div", class_="subTongzhiTextItemDate",recursive=True)
