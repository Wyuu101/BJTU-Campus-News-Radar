...
SECTION_ID = "section_83_01"
SECTION_NAME = "交通运输学院-通知公告"
...
def parse_page(
    ...
    for item in news_list.find_all("li", class_="list_down", recursive=False):
...
def _parse_list_item(item: BeautifulSoup, base_url: str) -> ResultSummary | None:
    content = item.find("div",class_="list_down_title",recursive=True)
    if content is None:
        return None
...
def _parse_date(item: BeautifulSoup) -> str | None:
    date_node_div = item.find("div", class_="list_down_btn")
    if date_node_div is None:
        return None

    date_node = date_node_div.find("a")
