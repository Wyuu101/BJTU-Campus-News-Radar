...
SECTION_ID = "section_85_09"
# 非全日制研究生培养
SECTION_NAME = "软件学院-非全日制研究生招生及培养"

BASE_URL = "https://sse.bjtu.edu.cn/cms/item/"
CATEGORY_ID = 79
MAX_PAGES = 1
...
def parse_page(
    ...
    news_list = soup.find("ul", class_="second_R_list")
    if news_list is None:
        logger.debug("第 %s 页未找到 ul.second_R_list", page)
        return []
...
def _parse_list_item(item: BeautifulSoup, base_url: str) -> ResultSummary | None:
    ...
    href_and_title_node = item.find("a",target="__blank",href=True)
